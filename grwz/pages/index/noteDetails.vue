    ...
    <div class="note-box">
      <div class="node-details" v-loading="loading">
        ...
        <div class="comment-box">
          <h3>以上是我的个人观点和理解，部份解决方案来源于网络，本网站仅供个人学习经验总结，如果您有更深的理解还望您不吝赐教，我的联系邮箱:1045749725@qq.com，您也可以在下方留言</h3>
          <textarea placeholder="积累跬步，可至千里，千里之行，始于足下，评论一条再走吧!" v-model="commentMsg"></textarea>
          <el-button type="primary" class="btn" @click="sendComment">发布评论</el-button>
        </div>
...
export default {
  ...
  async asyncData({ query={} }) {
     ...
     const result = await get(getNoteSeo, { note_id: id });
     return {
        details:result.data.list[0] || {}
     }
  },
  head() {
    let {title=this.$route.params.title || '笔记详情',desc=this.$route.params.desc||'笔记详情'} = this.details;
    return {
      title: `邵预鸿-${title}`,
      meta: [
        {
