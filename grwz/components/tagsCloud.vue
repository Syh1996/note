	...
	<div class="page">
		<div class="tagsCloud" :style="{ height: boxWidth + 'px', width: boxWidth + 'px' }" ref="Panel" @mousemove="listener($event)">
			<div
				...
				:key="index"
				:style="[
					{ opacity: (400 + item.z) / boxWidth },
					{ 'font-size': 12 * (boxWidth / (400 - item.z)) + 'px' },
					{ left: item.x + 'px' },
					{ top: item.y + 'px' },
...
export default {
	...
	methods: {
		...
		toUrl(url) {
			//window.location.href = "https://shaoyuhong.com/grzx/about/v1.pdf";
			window.open('https://shaoyuhong.com/grzx/about/v1.pdf');
		}
	},
	beforeDestroy(){
		clearInterval(this.timer1);
